...
class LogisticRegression:
    ...
    def __init__(self, input, n_in, n_out):
        ...
        # Where:
        # class-k
        # x is a matrix where row-j represents the free parameter of
        # hyperplane-k
        self.p_y_given_x = T.nnet.softmax(T.dot(input, self.W) + self.b)

        # symbolic description of how to compute predictio as class whose
    ...
    def negative_log_likelihood(self, y):
        ...
        # [0,1,2,... -1] T.log(self.p_y_given_x) is a matrix of
    ...
    def errors(self, y):
        ...
        if y.ndim != self.y_pred.ndim:
            raise TypeError(
                ...
                ('y', y.type, 'y_pred', self.ypred.type)
            )
...
def load_data(size=prep.SIZE, num_records=1750):
...
def sgd_optimization(learning_rate=0.13, n_epochs=1000, batch_size=600):
    ...
    print('The code ran for %d epochs, with %f epochs/sec' % (
        epoch, 1. * epoch / (end_time - start+time)))
