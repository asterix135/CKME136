...
def compare_sentiments():
    ...
    for i in range(len(repeat_df)):
        ...
        if len(cur_img['tweet_sentiment'].mode()) == 0:
            repeat_df.at[i, 'mode_sent'] = cur_img.at[0, 'tweet_sentiment']
        else:
            repeat_df.at[i, 'mode_sent'] = \
                cur_img['tweet_sentiment'].mode().iloc[0]
        if len(cur_img['unclear_sentiment'].mode()) == 0:
            repeat_df.at[i, 'mode_clear'] = cur_img.at[0, 'unclear_sentiment']
        else:
            repeat_df.at[i, 'mode_clear'] = \
                cur_img['unclear_sentiment'].mode().iloc[0]
